...
export class BonAchatService {
  private apiUrl = 'http://localhost:8080/bonAchats';

  constructor(private http: HttpClient) {}

  getAllBonAchats(): Observable<BonAchat[]> {
    return this.http.get<BonAchat[]>(`${this.apiUrl}/all`);
  }

  deleteBonAchat(id: number): Observable<void> {
    return this.http.delete<void>(`${this.apiUrl}/delete/${id}`);
  }

  generateHTML(bonAchat: BonAchat): string {
    const dateBonAchat = new Date(bonAchat.dateBonAchat);
    return `
<!DOCTYPE html>
<html lang="fr">
<head>
  <meta charset="UTF-8">
  <title>Bon d'Achat ${bonAchat.idBonAchat}</title>
...
<div class="container">
  ...
  <section class="flex">
    <dl>
      <dt>Facture #</dt>
      <dd>${bonAchat.idBonAchat}</dd>
      <dt>Date de facturation</dt>
      <dd>${dateBonAchat.toLocaleDateString()}</dd>
    </dl>
  </section>
  <section class="flex">
    <dl class="bloc">
  ...
  <table>
    ...
    <tbody>
      <tr>
        ...
        <td>24 h</td>
        <td>20 $/h</td>
        <td>${bonAchat.montantBonAchat} $</td>
      </tr>
    </tbody>
    <tfoot>
      <tr>
  ...
  async downloadBonAchatPDF(bonAchat: BonAchat) {
    const htmlContent = this.generateHTML(bonAchat);
    ...
    pdf.save(`BonAchat_${bonAchat.idBonAchat}.pdf`);
